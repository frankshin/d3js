L.HexLayer = L.Class.extend({
  ...
  _initContainer: function() {
    ...
    if (!this._container || overlayPane.empty) {
      ...
      this._container = d3.select(overlayPane).append('svg').attr('class', 'leaflet-layer leaflet-zoom-hide');
    }
  },
  ...
  _update: function() {
    ...
    this._container.attr("width", width).attr("height", height).style("margin-left", margin_left + "px").style("margin-top", margin_top + "px");
...
L.hexLayer = function(data, options) {
  const newL = new L.HexLayer(data, options);
  console.warn('newL:', newL)
  return newL
};